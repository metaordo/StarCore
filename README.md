...
- **[2026-08-08]** 🔥 StarCore 正式发布！首个基于系统状态链学习的网络空间世界认知模型，模型权重全面开源。

<span id='-模型介绍'/>

##  模型介绍

**StarCore** 是首个基于**系统状态链（State-Chain）学习**的网络空间世界模型，能够深度理解、动态预测、智能研判复杂长链安全风险。

网络系统世界中的多元数据链信息如网络流量、系统日志、代码执行、智能体轨迹等都可用于形成**状态链**，支撑世界模型的学习。StarCore 的核心构想是：
...
- **长链深度推演与安全分析**：支持32K上下文的系统数据长链分析推理，实现系统数据链分析预测、安全研判等任务。
...
首创网络空间状态链建模方法。基于可观测的数据链信息，世界模型 $G_{WM}$($s_t$, $a_t$) 将可观测的系统数据链定义为状态链 **{$s_0$, $s_1$, …, $s_t$}**，深度理解系统状态转换过程，捕获具有不同良性或攻击意图的系统状态转换动作序列 **{$a_0$, $a_1$, …, $a_{t-1}$}**。
...
| 任务 | 说明                         |
| --- |----------------------------|
| **状态链补全** | 给定 $s_0$…$s_t$，补全其中缺失的状态链条 |
| **状态链预测** | 基于历史状态链，预测未来状态或风险趋势        |
| **状态链安全研判** | 通过中间状态可解释推理，完成风险研判与溯源      |

模型架构包含**状态编码器、动态转移模型、观测解码器**，可推理系统运行状态转换信息 $A$ = {$a_0$, $a_1$, …, $a_{t-1}$}，并结合不同模态特点和系统运行环境，推理状态链所涉及的系统配置、用户行为、外部威胁情报等环境上下文信息。

### Step 3 · 理解世界：状态深度思考的强化学习（State-Chain RL）

提出**基于 GRPO 的状态链强化学习方法**，无需额外价值网络，以组内相对优势更新世界模型：

1. **状态链任务输入**：状态链 $s_0$, … , $s_t$ ＋ 分析问题 / 目标
2. **策略采样**：世界模型按当前策略采样生成 G 条候选推理链（Group Rollouts）
3. **精细化奖励评分**：多维奖励机制对每条推理链打分 $R_1$, $R_2$, …, $R_G$
4. **GRPO 策略更新**：以综合奖励 R = Σ wᵢ·Rᵢ 驱动策略迭代优化
...
| 模型 | 代码 Code | 智能体轨迹 Agent | 网络流量 Traffic | Linux 命令 Command | 安全研判 Security | 系统状态补全 Completion | 系统状态预测 Prediction | **综合评分 Overall** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
